 ...
 * Fragmento que representa el apartado del foro "Album"
 */
public class FragmentoForoAlbumes extends Fragment {
    //Declaramos los elementos necesarios
    FirebaseFirestore firestore;
    CollectionReference collectionReference_post, collectionReference_perfil;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View view = inflater.inflate(R.layout.fragment_fragmento_foro_albumes, container, false);
        ...
        //Instanciamos el objeto dfe la clase CollectionRefence donde le pasamos la colección "perfil"
        collectionReference_perfil = firestore.collection("perfil");

        //Creamos e instanciamos el objeto de la clase Query donde buscamos los "posts" donde la categoria de estos sean "Albumes"
        Query query_foro = collectionReference_post.whereEqualTo("categoria", "Albumes");
        ...
        query_foro.get().addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
            /**
             * Método donde realizaremos las acciones pertinentes en caso de que el proceso se haya completadfo de forma exitosa
             * @param task Objeto de la clase Task
             */
            @SuppressLint("MissingInflatedId")
            @Override
            public void onComplete(@NonNull Task<QuerySnapshot> task) {
                //Recoreremos los datos obtenidos y por cada dato, obtenemos el nombre de usuario
                for (QueryDocumentSnapshot documentSnapshot_foro : task.getResult()) {
                    ...
                    query_perfil.get().addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
                        /**
                         * Método donde realizaremos las acciones pertinentes en caso de que el proceso se haya completado de forma exitosa
                         * @param task Objeto de la clase Task
                         */
                        @Override
                        public void onComplete(@NonNull Task<QuerySnapshot> task) {
                            ...
                            recyclerView = view.findViewById(R.id.listadoForoAlbum);
